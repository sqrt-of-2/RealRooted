...
example (aa ab ac ba bb bc : ℝ) {d : ℕ} (hd : 2 ≤ d)
...
example (a0 a1 b1 b2 c2 : ℝ) {d : ℕ} (hd : 2 ≤ d)
...
example (a0 a1 b1 b2 c3 : ℝ) {d : ℕ} (hd : 2 ≤ d)
    (hdeg : (quadraticBidiagonalResidual
...
example {P : ℕ → ℝ[X]} {a0 a1 b1 b2 c2 : ℕ → ℝ}
    {degreeBound : ℕ → ℕ}
...
example {P : ℕ → ℝ[X]} {a0 a1 b1 b2 c3 : ℕ → ℝ}
    {degreeBound : ℕ → ℕ}
    (hbase : IsPFPolynomial (P 0))
    (hdegree : ∀ n, (P n).natDegree ≤ degreeBound n)
    (hd : ∀ n, 2 ≤ degreeBound n)
